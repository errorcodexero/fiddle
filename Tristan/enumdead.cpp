...
#define ENDTWO 0
using namespace std;
//1.Non-repeating direction
//2.fuction for directions for the robot to acheave
/////////////////////////////////////////////////////// //////////////////////
// Structures
//
// All Structures
...
////////////////////////////////////////////////////////////////////////////
vector<pair<int,movedir>> findlist2(direction crdir,vector<pair<int,direction>> v){
...
int main(){
	...
	*/
	...
	f.x = STARTONE - 1;//set
	f.y = STARTTWO - 1;//inital location for previus

	p.x = STARTONE;//set px
	p.y = STARTTWO;//location of start point py

	e.x = ENDONE;//set ex
	e.y = ENDTWO;//location of end point ey
	...
	for(unsigned int i = 0; i < ipath.size()-1;i++){ //loop through the vector and deturmine the route 
		...
		//drivec.push_back(turn);//store the infromation in a vector
		//pear = findlist(drivec);//con 
		//.currentdir=nextdir; //moves to next space
	}
	...
	pear2 = findlist2(currentdir,pear);
	
	cout << "Finnal" << pear2 << endl;
